// -*- mode: cpp; mode: fold -*-
// Description								/*{{{*/
// $Id: version.cc,v 1.4 2000/09/04 15:55:08 kojima Exp $
/* ######################################################################
...
// Include Files							/*{{{*/
#ifdef __GNUG__
#pragma implementation "apt-pkg/systemfactory.h"
#endif 
...
// ---------------------------------------------------------------------
/* */
int SystemFactory::versionCompare(const char *A, const char *B)
{
...
/* This fragments the version into E:V-R triples and compares each 
   portion seperately. */
int SystemFactory::versionCompare(const char *A, const char *AEnd, const char *B,
				  const char *BEnd)
{
    ...
    if (drhs != rhs)
	drhs++;
    Res = iVersionCompare(dlhs,AEnd,drhs,BEnd);
...
/* This simply preforms the version comparison and switch based on 
   operator. */
bool SystemFactory::checkDep(const char *DepVer,const char *PkgVer,int Op)
{
   if (DepVer == 0)
      return true;
   if (PkgVer == 0)
   ...
   int Res = versionCompare(PkgVer,DepVer);
   switch (Op & 0x0F)
   {
...
string SystemFactory::baseVersion(const char *Ver)
{
